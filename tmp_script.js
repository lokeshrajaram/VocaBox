    ...
    function renderPracticeItem(session) {
      ...
      card.innerHTML = `
        <div>
          <div class=\"flex justify-between items-center\">
            ...
            <button class=\"btn btn-ghost say-word\">🔊 Play</button>
          </div>
      ...
      const evaluate = ()=>{
        ...
        if (spellFb) {
          spellFb.textContent = ok1 ? '✅ Correct' : `❌ It was \"${session.current.word}\"`;
        ...
        if (defFb) {
          defFb.textContent = ok2 ? '✅ Correct' : '❌ Incorrect';
      ...
      if (checkBtn) {
        checkBtn.addEventListener('click', ()=>{
          ...
          list.querySelectorAll('.fill-card').forEach(card=>{
            ...
            if (fb) {
              fb.textContent = ok ? '✅ Great choice!' : `❌ The missing word was \"${word}\"`;
      ...
      sum.innerHTML = `
        <div class=\"relative overflow-hidden rounded-[32px] border-[6px] border-amber-200 bg-gradient-to-br from-amber-50 via-white to-indigo-50 shadow-2xl px-6 py-10 sm:px-12\">
          ...
          <div class=\"relative mt-10 flex flex-col items-center gap-3 text-center\">
            ...
            <p class=\"text-sm text-gray-500\">Start another ${encouragement} anytime by choosing sets above—keep the momentum going!</p>
          </div>
        </div>`;
      activeSessionKey = null;
    }
